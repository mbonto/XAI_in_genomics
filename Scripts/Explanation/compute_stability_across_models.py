...
argParser.add_argument("-m1", "--model1", type=str, help="model name (LR, MLP, DiffuseLR, DiffuseMLP)")
argParser.add_argument("-m2", "--model2", type=str, help="model name (LR, MLP, DiffuseLR, DiffuseMLP)")
...
print('Dataset    ', name)
print('Models    ', model1_name, model2_name)
...
# Load the attributions of all the examples of the set for model
exps = np.arange(1, n_repet + 1)

def load_average_attributions(model_name, exps, XAI_method, method, save_path, set_name):
    attr_exps = {}
    arg_exps = {}
    for exp in exps:
...
arg1_exps = load_average_attributions(model1_name, exps, XAI_method, method, save_path, set_name)
arg2_exps = load_average_attributions(model2_name, exps, XAI_method, method, save_path, set_name)
    ...
    for exp1 in exps:
        for exp2 in exps:
            if n == 100:
                print(exp1, exp2)
            scores = []
            for _class in arg1_exps[exp1].keys():
                # print("class", _class)
                arg1 = set(arg1_exps[exp1][_class][:n])
                arg2 = set(arg2_exps[exp2][_class][:n])
...
np.savez(os.path.join(save_path, "figures", "Jaccard_scores_between_{}_and_{}_with_{}_on_{}".format(model1_name, model2_name, XAI_method, set_name)), n_args, avg_jaccard_scores)
...
plt.savefig(os.path.join(save_path, "figures", "Jaccard_scores_between{}_and_{}_with_{}_on_{}".format(model1_name, model2_name, XAI_method, set_name)), bbox_inches='tight', dpi=150)
