...
selections = {"LR": "IG_LR_set_train_", "MLP":"IG_MLP_set_train_", "GCN":"IG_GCN_set_train_", "LR_L1_penalty":"LR_"}
...
for selection_type in ["best", "worst"]:
    ...
    for n_feat in n_feats:
        ...
        data = []
        for exp in exps:
            save_name = os.path.join(model_name, "FS", f"{exp}_{selections[model_name]}exp_{exp}_{n_feat}_{selection_type}.csv")
...
n_rand = 3
for selection_type in ["random_wo_best", None]:
    results[selection_type] = {}
    for n_feat in n_feats:
        results[selection_type][n_feat] = {}
        data = []
        for exp in exps:
            data_r = []
            for r in range(1, n_rand+1):
                save_name = os.path.join(model_name, "FS", f"{r}_None_{n_feat}_{selection_type}.csv") if selection_type is None else os.path.join(model_name, "FS", f"{r}_{selections[model_name]}exp_{exp}_{n_feat}_{selection_type}.csv")
                with open(os.path.join(save_path, save_name), "r") as f:
                    lines = f.readlines()
                    for line in lines:
                        line = line.strip().split(', ')
                        if line[0] == 'balanced_test':
                            data_r.append(float(line[1]))
            assert len(data_r) == n_rand
            data.append(np.mean(data_r))
        assert len(data) == n_repet
        results[selection_type][n_feat]["mean"] = np.round(np.mean(data), 2)
        results[selection_type][n_feat]["std"] = np.round(np.std(data), 2)
